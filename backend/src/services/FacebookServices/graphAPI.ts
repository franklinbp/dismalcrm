...
const GRAPH_VERSION =
  process.env.META_GRAPH_VERSION || process.env.FACEBOOK_GRAPH_VERSION || "v13.0";

const getMetaAppId = (): string | undefined =>
  process.env.META_APP_ID || process.env.FACEBOOK_APP_ID;

const getMetaAppSecret = (): string | undefined =>
...
const getConfiguredMetaAppId = async (): Promise<string | undefined> =>
  (await getSettingValue("metaAppId")) || getMetaAppId();

const getConfiguredMetaAppSecret = async (): Promise<string | undefined> =>
  (await getSettingValue("metaAppSecret")) || getMetaAppSecret();
...
export const getAccessToken = async (): Promise<string> => {
  const { data } = await axios.get(
    ...
    {
      params: {
        client_id: await getConfiguredMetaAppId(),
        client_secret: await getConfiguredMetaAppSecret(),
...
export const profilePsid = async (id: string, token: string): Promise<any> => {
  try {
    const { data } = await axios.get(
      `https://graph.facebook.com/${GRAPH_VERSION}/${id}?access_token=${token}`
    );
    return data;
  } catch (error) {
    console.log(error);
...
export const subscribeApp = async (id: string, token: string): Promise<any> => {
  try {
    const { data } = await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${id}/subscribed_apps?access_token=${token}`,
      {
        subscribed_fields: [
          "messages",
          "messaging_postbacks",
...
export const removeApplcation = async (
  ...
  try {
    await axios.delete(`https://graph.facebook.com/${GRAPH_VERSION}/${id}/permissions`, {
      params: {
        access_token: token
      }
    });
